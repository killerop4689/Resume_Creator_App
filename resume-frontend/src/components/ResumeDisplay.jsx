...
//     console.log("Resume being copied:", resume);
//       const textContent = `
...
//     PROJECTS
//     ${resume.projects?.map(p => `${p.title}${p.technologies?.length ? ` (${p.technologies.join(", ")})` : ""}\n${p.bullet_points?.map(b => `• ${b}`).join("\n")}`).join("\n\n")}

//     EDUCATION
//     ${resume.education?.join("\n")}
//       `.trim();
//       console.log("Text being copied:", textContent);
